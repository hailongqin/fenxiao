...
export default {
	...
	onLoad() {
		...
		this.$root.get("/basic/plugin/form/content/obtain", { formId: id, token: token }, (data) => {
			if (data.success) {
				...
				that.formContentId = data.formContent.id
		...
		cancel(e) {
			...
			wx.showModal({
				...
				success: function(res) {
					if (res.confirm) {
						$this.$root.post("/basic/plugin/form/content/cancel", { id: $this.formContentId, token: wx.getStorageSync("token") }, function(data) {
							if (data.success) {
								wx.showToast({
									title: "取消预约成功",
									icon: 'success',
