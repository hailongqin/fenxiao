...
//  	}
        ...
        wx.request({
            url: this.$root.apiServer+this.$root.variate +this.$root.appid+'/basic/client/shoppingCart/cart/listProduct/',
            data: {
                token: that.$root.globalData.token
            },
            method: 'GET',
    ...
    methods: {
        ...
        findItemIndexByUserId (userId) {
            if (userId >= 0) {
                for (var i = 0, len = this.items.length; i < len; i++) {
                    if (this.items[i].shoppingCartId == userId)
                    return i
                }
            }
        },
        ...
        changeNum (userId, isPlus) {  
            ...
            if (isPlus){//加
                num++
                    wx.request({
                        url: this.$root.apiServer+this.$root.appid+'/basic/client/shoppingCart/cart/updateStock',
                        data: {
        ...
        postDel (data) {
            ...
            if (data) {
                var that = this
                wx.request({
                    url: this.$root.apiServer+this.$root.appid+'/basic/client/shoppingCart/cart/batchRemoveProduct',
                    data: {
                        shopIdList: data,
                        token: getApp().globalData.token,
                    },
            ...
            checkoutHandler() {
	    			...
	    			if(to.length) {
	    				...
	    				for(var i=0;i<to.length;i++){
	    					if(to[0].type !== to[i].type)//发现一个不等，改为false
	    					{
	    						isAllEqual=false
	    						break;//跳出循环
	    					}
...
<style scoped>
page{background:#f9f9f9;padding-bottom: 120rpx}
.page{background:#f9f9f9;}

.auto{width: 690rpx}
.items{position: relative; width: 100%;height: 236rpx;margin-top: 20rpx;overflow: hidden;background: #fff}
.scroll{height: 236rpx;white-space: nowrap;display: flex;position: absolute; top:0;left:0;width:100%;z-index: 10}
.goods-item{background:#ffffff; width: 100%; height: 236rpx; display: inline-block;overflow: hidden;}

.good-prod{width: 100%;height: 236rpx;}
.good-cant{margin-top: 30rpx;}
.goods-check,.goods-img,.goods-text{float: left;margin-right: 24rpx}
.goods-check{margin-top: 68rpx;position: relative}
.item-checkbox{opacity: 0;position: absolute;left: 0;top: 0;width: 40rpx;height: 40rpx;z-index: 99}
.label-icon {position: relative;display: inline-block;vertical-align: middle;width: 36rpx;height: 36rpx; border:solid 2rpx #c9c9c9;border-radius: 50%;}
.label-icon-checked { position: absolute;top:-1rpx;left:-1rpx;width:40rpx;height: 40rpx;border-radius: 50%}
.label-icon-checked .label-icon-checked-image{position: absolute;width:40rpx;height: 40rpx; }

.goods-img{width: 176rpx; height: 176rpx;}
.empty{border:solid 2rpx #e3e3e3;font-size:30rpx;line-height: 140rpx;text-align: center }

.goods-img .goods-img-image{width: 176rpx; height: 176rpx;}
.goods-text{font-size: 26rpx;width: 300rpx;}
.goods-name{white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color: #333333;}
.goods-norms{margin-top: 18rpx; color: #999;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
...
.stepper {width: 158rpx;height: 36rpx;border: 1px solid #ccc;border-radius: 4rpx;line-height: 36rpx;position: absolute;right:30rpx ;bottom:30rpx;}
...
.good-del{width: 120rpx;position: absolute;right: 0;top: 0;z-index: 9;height: 236rpx;background: #fff}
.good-b{text-align: center;color:#ffffff;width: 120rpx;height: 236rpx;line-height: 236rpx;}
...
.groupTxt{line-height: 40rpx;margin-top: 35rpx;margin-left: 24rpx;}
.setText{font-size: 24rpx;text-align: right;margin-right: 30rpx;line-height: 100rpx;}
.count{color: #999999;margin-right: 8rpx;}
