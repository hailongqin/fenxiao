<template>
    <view class="page">
        <view class="pos-f-t" wx:if="{{topics.list.length>0}}">
            <view class="top plr30 box-s" @tap="lookPost(topics.list[0].postId)">
                <view class="ft28 c007 container">【帖子】{{topics.list[0].postTitle}}</view>
            </view>
        </view>
            ...
            <view class="content box-s">
                ...
                <view class="recommend_content_count display-flex">
                    <view class="reply pos-r ">
                        <image class="reply_image pos-a"  src="http://image.vdongchina.com/M00/03/3A/ZSUkmVngGcqEBBI8AAAAAP6Kn4Q944.png" />
                        <text class="reply_count pos-r" @tap="recoverFollow(item.followId)">恢复</text>
                    ...
                    <view class="reply pos-r">
                        ...
                        <text class="reply_count pos-r" @tap="deleteFollow(item.followId)">彻底删除</text>
                    </view>
                </view>
            </view>
...
<script>
var GetList = function(that) { //请求删除跟帖列表
    that.$root.get("/basic/community/app/follow/page", { page: that.page, postId: that.postId, size: 10, currentUserId: that.userId ,deleted:true}, (data) => {
        if (data.success) {
            that.loadMore = false
    ...
    methods: {
        ...
        recoverFollow(followId) {
            ...
            wx.showModal({
                ...
                success: function(res) {
                    if (res.confirm) {
                        that.$root.post("/basic/community/app/follow/recover", { followId: followId }, function(data) {
                            if (data.success) {
                                that.hidden = false
                                that.followList = []
                                that.isAll = false
        ...
        deleteFollow(followId) {
            ...
            wx.showModal({
                ...
                success: function(res) {
                    if (res.confirm) {
                        that.$root.post("/basic/community/app/follow/deletePhysically", { followId: followId }, function(data) {
                            if (data.success) {
                                that.hidden = false
                                that.followList = []
                                that.isAll = false
