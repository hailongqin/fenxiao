    ...
    <view class="page">
        <form @submit="formSubmit" class="bg-wt">
            <view class="plr30 box-s bor-b bg-wt">
                <textarea class="category_content ft32" name="content" maxlength="-1" placeholder="请输入跟帖内容" placeholder-class="ft32 c9" />
            </view>
            ...
            <view class="upload plr30 box-s display-flex" wx:if="{{ishtml}}">
                <view class="previewImgs pos-r mb30" wx:for="{{previewImg}}" wx:key="files">
                    <image class="previewImg" src="{{item}}"></image>
    ...
    methods: {
                    ...
                    if (file) {
                        ...
                        xhr.onload = function() {
                            ...
                            if (errorMsg.success) {
                                ...
                                previewImgs.push(that.imgUrl + errorMsg.fileid)
                            }
                        }
                        xhr.send(form);
                        that.files = filearr
        ...
        formSubmit(e) {
            ...
            if (that.count == 0) {
                ...
                data["imgList"] = that.previewImg
                that.$root.post("/basic/community/app/follow/save", data, function(data) {
                    if (data.success) {
                        if ((permission & 16) > 0) {
                            wx.redirectTo({
                                url: '../threadAudit/threadAudit?followId=' + data.data,
                            })
                            return;
                        } else {
                            wx.navigateBack({
