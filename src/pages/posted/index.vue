    ...
    <view class="page">
        <form @submit="postInvitation" class="bg-wt">
            <view class="name plr30 box-s bg-wt">
                <input class="name_input ft32" name='title' placeholder="请输入标题" placeholder-class="c9" />
            </view>
            <view class="category plr30 box-s display-flex bg-wt">
                ...
                <block wx:for="{{categoryList}}" wx:key="categoryList">
                    <text wx:if="{{item.display == 1}}" class=" {{plateId == item.plateId ? 'category_title' :'category_titleon'}}" @tap="category(item.plateId)">{{item.plateName}}</text>
                </block>
            </view>
            <view class="plr30 box-s bor-b bg-wt">
                <textarea class="category_content ft32" name="content" maxlength="-1" placeholder="请输入正文内容" placeholder-class="test" />
            </view>
    ...
    methods: {
            ...
            if (typeof window != 'undefined') {
                ...
                input.onchange = function(e) {
                    ...
                    if (file) {
                        ...
                        xhr.open("post", that.$root.globalData.upImage, true);//设置提交方式，url，异步提交
                        xhr.onload = function() {
        ...
        postInvitation(e) {
            ...
            if (that.count == 0) {
                ...
                that.$root.post("/basic/community/app/post/save", data, function(data) {
                    if (data.success) {
                        if ((permission & 8) > 0) {
                            wx.redirectTo({
                                url: '../commentAudit/commentAudit?postId=' + data.data,
                            })
                            return;

                        } else {
                            wx.redirectTo({
                                url: '../topics/topics?id=' + data.data,
                            })
                            return;
                        }
                    } else {
